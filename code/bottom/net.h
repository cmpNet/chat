...
int send_packet(char* packet, in_addr_t ip, int port) {
	...
	if (sendto(s, packet, PACKETLEN, 0, (struct sockaddr*)&sin, sizeof(sin)) < 0) {
		close(s);
		return -1;
	} else {
		//printf("Send Pac\n");
...
int my_recv(tcb_t* tcpblock) {
	int count = 0;
	int revfd = socket(PF_PACKET, SOCK_DGRAM, htons(ETH_P_IP));
	if (revfd == -1) {
	...
	while (1) {
		ssize_t n = recv(revfd, tcpblock->buffer, sizeof(tcpblock->buffer), 0);
		if (n == -1) {
	    printf("recv error!\n");
	    return -1;
	  } else if (n == 0) {
...
int send_syn_ack(in_addr_t our_ipaddr, in_addr_t their_ipaddr, int our_port, int their_port, int seqnr) {
	char* packet = create_packet(our_ipaddr, their_ipaddr, SYN_ACK, seqnr, our_port, their_port, "", 0);

	if (send_packet(packet, their_ipaddr, their_port) < 0) {
...
int send_syn(in_addr_t our_ipaddr, in_addr_t their_ipaddr, int our_port, int their_port, int seqnr) {
...
int send_ack(in_addr_t our_ipaddr, in_addr_t their_ipaddr, int our_port, int their_port, int seqnr) {
...
int send_data(in_addr_t our_ipaddr, in_addr_t their_ipaddr, int our_port, int their_port, int seqnr, char* data, int len) {
	char* packet = create_packet(our_ipaddr, their_ipaddr, NONE, seqnr, our_port, their_port, "", 0);
